...
namespace Escrow.Api.Application.UserPanel.Commands
{
    ...
    public class UpdateMobileNumberCommandHandler : IRequestHandler<UpdateMobileNumberCommand, Result<object>>
    {
        ...
        public async Task<Result<object>> Handle(UpdateMobileNumberCommand request, CancellationToken cancellationToken)
        {
            ...
            {
                return Result<object>.Failure(StatusCodes.Status400BadRequest, AppMessages.Get("InvalidRequestData", language));
            }
            ...
            if (!isPhoneNumberValid)
                return Result<object>.Failure(StatusCodes.Status400BadRequest, AppMessages.Get("InvalidPhoneNumberFormat", language));
            ...
            return Result<object>.Success(StatusCodes.Status200OK, AppMessages.Get("OtpSentToMobile", language), new { MobileNumber = phoneNumber, UserId = user.Id });
        }
    }
}
