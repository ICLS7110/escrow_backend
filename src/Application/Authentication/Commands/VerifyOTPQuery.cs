...
namespace Escrow.Api.Application.BankDetails.Queries
{
    ...
    public class VerifyOTPHandler : IRequestHandler<VerifyOTPQuery, Result<VerifyOtpDto>>
    {
        ...
        public VerifyOTPHandler(IOtpManagerService otpManagerService, IUserService userService, IJwtService jwtService, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<Result<VerifyOtpDto>> Handle(VerifyOTPQuery request, CancellationToken cancellationToken)
        {
            ...
            if (!isValid)
            {
                return Result<VerifyOtpDto>.Failure(StatusCodes.Status404NotFound, AppMessages.Get("InvalidOtp", language));
            }
            ...
            if (res.Data is null)
            {
                return Result<VerifyOtpDto>.Failure(StatusCodes.Status404NotFound, AppMessages.Get("NotFound", language));
            }
            ...
            return Result<VerifyOtpDto>.Success(StatusCodes.Status200OK, AppMessages.Get("OtpVerified", language), result);
        }
    }
}
...
//        var isValid = _otpManagerService.VerifyOtpAsync(phoneNumber, request.Otp);
//        if (!isValid)
//            return Result<VerifyOtpDto>.Failure(StatusCodes.Status404NotFound,AppMessages.InvalidOtp);
...
//        if (res.Data is null)
//            return Result<VerifyOtpDto>.Failure(StatusCodes.Status404NotFound, AppMessages.NotFound);
...
//            IsProfileCompleted=res.Data.IsProfileCompleted
//        };
//        return Result<VerifyOtpDto>.Success(StatusCodes.Status200OK, AppMessages.OtpVerified, result);
