...
namespace Escrow.Api.Application.ContactUsPanel.Commands
{
    ...
    public class SendContactMessageHandler : IRequestHandler<SendContactMessageCommand, Result<int>>
    {
        ...
        public SendContactMessageHandler(IApplicationDbContext context, IEmailService emailService, IHttpContextAccessor httpContextAccessor)
        {
            _context = context;
        ...
        public async Task<Result<int>> Handle(SendContactMessageCommand request, CancellationToken cancellationToken)
        {
            try
            {
                // Get the current language from the HttpContext
                ...
                if (result == 0)
                {
                    return Result<int>.Failure(StatusCodes.Status500InternalServerError, AppMessages.Get("UnableToSaveMessageToDatabase", language));
                }
                ...
                return Result<int>.Success(StatusCodes.Status200OK, AppMessages.Get("MessageSent", language), contactMessage.Id);
            }
            catch (Exception ex)
            {
...
//            if (result == 0)
//            {
//                return Result<int>.Failure(StatusCodes.Status500InternalServerError, AppMessages.Unabletosavemessagetothedatabase);
//            }

//            // Send email only after successful save
//            await _emailService.SendEmailAsync(request.Email, request.Title, request.Name, request.Message);

//            return Result<int>.Success(StatusCodes.Status200OK, AppMessages.Messagesent, contactMessage.Id);
//        }
//        catch (Exception ex)
//        {
//            // You can also log the exception here
