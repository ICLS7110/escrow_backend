...
public class CreateTransactionCommandHandler : IRequestHandler<CreateTransactionCommand, Result<object>>
{
    ...
    public async Task<Result<object>> Handle(CreateTransactionCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            return Result<object>.Failure(StatusCodes.Status401Unauthorized, AppMessages.Get("UserNotAuthenticated", language));
        }
        ...
        {
            return Result<object>.Failure(StatusCodes.Status404NotFound, AppMessages.Get("BuyerOrSellerNotFound", language));
        }
        ...
        {
            return Result<object>.Failure(StatusCodes.Status401Unauthorized, AppMessages.Get("UnauthorizedTransaction", language));
        }
        ...
        foreach (var user in users)
        {
            try
            {
                ...
                {
                    var subject = AppMessages.Get("TransactionCreatedEmailSubject", language);
                    var body = string.Format(AppMessages.Get("TransactionCreatedEmailBody", language), user.FullName, contract.Id);
            ...
            {
                errorMessages.Add($"{AppMessages.Get("FailedToNotifyUser", language)} {user.FullName}: {ex.Message}");
            }
        }
        ...
        var successMessage = AppMessages.Get("TransactionCreatedSuccessfully", language);

        if (errorMessages.Any())
        {
            var fullMessage = successMessage + "\n" + AppMessages.Get("WithWarnings", language) + "\n" + string.Join("\n", errorMessages);
