...
namespace Escrow.Api.Application.Notifications.Queries
{
    ...
    public class GetAllNotificationsQueryHandler : IRequestHandler<GetAllNotificationsQuery, Result<PaginatedList<NotificationDTO>>>
    {
        ...
        public GetAllNotificationsQueryHandler(IApplicationDbContext context, IJwtService jwtService, IHttpContextAccessor httpContextAccessor)
        {
            _context = context;
        ...
        public async Task<Result<PaginatedList<NotificationDTO>>> Handle(GetAllNotificationsQuery request, CancellationToken cancellationToken)
        {
            ...
            catch (Exception ex)
            {
                ...
                return Result<PaginatedList<NotificationDTO>>.Failure(StatusCodes.Status500InternalServerError, ex.Message);
            }
            ...
            return Result<PaginatedList<NotificationDTO>>.Success(
                ...
                string.Format(AppMessages.Get("NotificationsRetrievedSuccessfully", language), unreadCount),
                paginatedList);
        }
    }
}
