...
namespace Escrow.Api.Application.Notifications.Commands
{
    ...
    public class DeleteNotificationCommandHandler : IRequestHandler<DeleteNotificationCommand, Result<object>>
    {
        ...
        public DeleteNotificationCommandHandler(IApplicationDbContext context, IJwtService jwtTokenService, IHttpContextAccessor httpContextAccessor)
        {
            _context = context;
        ...
        public async Task<Result<object>> Handle(DeleteNotificationCommand request, CancellationToken cancellationToken)
        {
            ...
            if (request.Id.HasValue && request.Id.Value > 0)
            {
                if (request.Id <= 0)
                {
                    return Result<object>.Failure(StatusCodes.Status400BadRequest, AppMessages.Get("InvalidNotificationId", language));
                }

                var notification = await _context.Notifications.FindAsync(new object[] { request.Id.Value }, cancellationToken);
                if (notification == null)
                {
                    return Result<object>.Success(StatusCodes.Status200OK, AppMessages.Get("NotificationNotFound", language), new());
                }
                ...
                return Result<object>.Success(StatusCodes.Status200OK, AppMessages.Get("NotificationDeletedSuccessfully", language), new
                {
            ...
            {
                return Result<object>.Success(StatusCodes.Status200OK, AppMessages.Get("NoNotificationsFound", language), new());
            }

            _context.Notifications.RemoveRange(notifications);
            await _context.SaveChangesAsync(cancellationToken);

            return Result<object>.Success(StatusCodes.Status200OK, AppMessages.Get("AllNotificationsDeletedSuccessfully", language), new
            {
